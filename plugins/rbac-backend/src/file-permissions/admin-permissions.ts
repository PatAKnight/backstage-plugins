...
export class AdminPermission {
  private readonly adminUsers: Config[] | undefined;
  private readonly superUsers: Config[] | undefined;
  private readonly superUserList: string[] = [];
  private readonly configApi: ConfigApi;
  private readonly logger: LoggerService;

  private constructor(configApi: ConfigApi, logger: LoggerService) {
    this.configApi = configApi;
    this.logger = logger;
    this.adminUsers = this.configApi.getOptionalConfigArray(
      'permission.rbac.admin.users',
    );
    this.superUsers = configApi.getOptionalConfigArray(
      'permission.rbac.admin.superUsers',
    );

    if (this.superUsers && this.superUsers.length > 0) {
      for (const user of this.superUsers) {
        const userName = user.getString('name');
        this.superUserList.push(userName);
      }
    }

    if (
      (!this.adminUsers || this.adminUsers.length === 0) &&
      (!this.superUsers || this.superUsers.length === 0)
    ) {
      this.logger.warn(
  ...
  getAdminRoleMetadata = (): RoleMetadataDao => {
